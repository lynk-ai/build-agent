...
- **Content rules** — apply every rule in `references/content-rules.md` against the target files: single source of truth (Rule 1), correct placement per the docs (Rule 2), description clarity and red flags (Rule 4), cross-file consistency (Rule 5), reference integrity (Rule 6), engine compatibility (Rule 7), Lynk SQL syntax (Rule 8), domain coherence (Rule 9), examples-&-evaluations validity (Rule 10), and entity-key integrity (Rule 11). Use the detection rule number in the report's `local/<check-group>` tag — e.g. `local/content-rules-2` for a misplaced metric definition, `local/content-rules-7` for `QUALIFY` in a non-Snowflake engine, `local/content-rules-8` for `{feature_name}` curly braces inside an `expected_output` block, `local/content-rules-9` for an off-topic section in a domain knowledge file, `local/content-rules-10` for a private `_`-prefixed feature referenced in generated SQL, `local/content-rules-11` for an entity `keys:` that doesn't uniquely identify a row. The quick check at the bottom of `content-rules.md` is the minimum coverage; nothing in the target files should be skipped.
...
- **Examples & evaluations quality (Rule 10)** — covers every entry under `examples:` in entity YAMLs, every test case in `evaluations.yml`, **and** every SQL example embedded in task-instruction / knowledge markdown (the agent learns its query patterns from all three). The agent reuses their shape as in-context patterns, so a broken or misaligned one trains the agent on a wrong pattern. Apply all five points of Rule 10. Four sub-checks:
  - **Validity & queryability** [`local/content-rules-10`] — *static, no warehouse needed.* For each SQL: it parses in the detected engine dialect (Rule 7), uses the canonical generated-SQL surface (bare entity/features, `METRIC('x') AS x`, no `{…}` braces — Rule 8), and every entity / feature / metric it references **exists and is queryable** (Rule 6a + Rule 10 point 3). To catch non-queryable references mechanically, build the set of `name:` values that begin with `_` for each entity, then flag any **bare** occurrence of one in a `SELECT` / `WHERE` / `GROUP BY` / `IFF(...)` / aggregate argument — private `_`-prefixed features (`_is_…`, `_key`) resolve only inside feature-definition `{…}` SQL and make the engine fail to plan the query. **Severity: `error`.** This is the check that static reference-integrity (Rule 6a) alone misses and that, before this rule existed, only surfaced at runtime.
  - **Input ↔ expected_output coherence** [`local/examples-quality`] — the SQL's filters, groupings, metric/feature selections, and time window match what the `input` actually asks. *"How many active customers this month?"* with SQL that doesn't filter on `status` or doesn't restrict the date range is a drift. **Severity: `warning`**, escalate to **`error`** when the divergence changes which entity / metric / dimension the case is testing.
  - **Description ↔ test alignment** [`local/examples-quality`] — the `description` (entity examples) or test-case description states what the case is actually testing. *"Tests the refund-rate filter"* paired with SQL that doesn't reference refunds is misaligned. **Severity: `warning`**. Pure description-quality red flags (tautological, placeholder) belong under Rule 4 instead.
  - **Default-filter consistency** [`local/content-rules-5`] — every default filter the entity's task-instructions declare for that question type appears in `expected_output`. If task-instructions say *"always exclude `is_test_account = true`"* and the SQL doesn't, that's a Rule 5 contradiction — **needs-client-input** if the omission might be intentional (e.g., the case is specifically testing the included-test-accounts path), otherwise **`error`**.
...
- `local/content-rules-<N>` — raised by a content rule in Step 6. `<N>` is the rule number from `references/content-rules.md`: `1` single-source, `2` placement, `4` description-clarity, `5` consistency, `6` reference-integrity, `7` engine-compatibility, `8` lynk-sql-syntax, `9` domain-coherence, `10` examples-&-evaluations-validity (dialect / surface / queryability of an example or evaluation — including a private `_`-prefixed feature referenced in generated SQL), `11` entity-key-integrity (a declared `keys:` that doesn't uniquely identify a row — statically suspected in Step 6, confirmed by the Step 7 count probe). Rule 3 is action protocol (how to act on a Rule 2 finding), not a detection — misplacement findings get tagged `content-rules-2` and cite Rule 3 in the suggested fix.
