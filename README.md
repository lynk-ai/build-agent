...
The marketplace exposes one plugin, `lynk-semantic-layer`, which ships three skills:
...
- **`lynk-evaluate`** — judge whether a semantic layer is good enough for the AI agent: description quality, cross-file consistency, content placement, reference integrity, and SQL dialect compatibility against your warehouse engine. Local checks only; produces a prioritized report.
...
All three skills are model-invoked: describe what you want in natural language (for example, "add a `points_per_game` metric to the player entity", "evaluate my semantic layer", or "validate on the inquiries branch") and Claude will pick the right skill.
